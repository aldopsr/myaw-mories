...
import { ArrowLeft, ShoppingBag, Star, Share2, Instagram, Heart, CheckCircle2, Sparkles } from "lucide-react";
...
const PRODUCTS = {
  1: {
    ...
    description: "Each page is a canvas for your dearest memories. Hand-decorated with washi tape, sparkles, and floral motifs to turn your photos into a physical masterpiece.",
    features: [
...
export default function ProductDetail({ productId, onBack }: { productId: number, onBack: () => void }) {
  const [selectedSize, setSelectedSize] = useState("15x15 cm");
  const [selectedLevel, setSelectedLevel] = useState("Semi (Tape + Florals)");
  
  const product = PRODUCTS[1]; // Using the scrapbook as default detail for demo
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-6">
            <motion.div 
               ...
               className="polaroid p-6 pb-12 rotate-[-1deg]"
            >
            ...
            <div className="flex gap-4">
               {[1, 2, 3].map((i) => (
                 <div key={i} className={`w-24 h-24 polaroid p-2 pb-6 cursor-pointer hover:scale-110 transition-transform ${i === 1 ? 'rotate-3 border-primary' : i === 2 ? '-rotate-6' : 'rotate-2'}`}>
          ...
          >
            <div className="inline-block mb-4">
              <span className="label-sm text-primary tracking-widest font-extrabold flex items-center gap-2">
            ...
            <p className="body-lg text-on-surface-variant mb-8 leading-relaxed">
              {product.description}
            </p>

            <div className="p-8 bg-white/50 border-2 border-dashed border-primary-container rounded-[2rem] mb-10 relative overflow-hidden">
               <div className="flex justify-between items-end mb-8 relative z-10">
                  ...
                  <button className="flex items-center gap-2 text-primary hover:underline font-bold text-sm">
                     <Share2 size={16} /> Share Treasure
                  </button>
               </div>
                     ...
                     {product.sizes.map((size) => (
                       <button
                         ...
                         className={`px-6 py-2.5 rounded-full text-xs font-bold transition-all ${
                           selectedSize === size ? "bg-primary text-white" : "bg-white text-on-surface-variant border border-surface-container-highest"
                         }`}
                       >
                         {size}
                       </button>
                     ...
                     {product.levels.map((level) => (
                       <button
                         ...
                         className={`px-3 py-4 rounded-2xl text-[10px] font-bold text-center leading-tight transition-all border-2 ${
                           selectedLevel === level ? "border-primary bg-primary-container/20 text-primary" : "border-surface-container-highest bg-white text-on-surface-variant"
                         }`}
                       >
                         {level.split('(')[0].trim()}
               ...
               <div className="space-y-3 mb-8">
                  {product.features.map((feature, i) => (
                    <div key={i} className="flex items-center gap-3 text-xs font-medium text-on-surface-variant">
                       <CheckCircle2 size={16} className="text-primary-container" /> {feature}
                    </div>
                  ))}
               </div>
               ...
               >
                 Order via Instagram <ArrowLeft className="rotate-[135deg]" size={20} />
               </motion.button>
            ...
            <div className="flex items-center justify-center gap-8 text-on-surface-variant">
               <button className="flex items-center gap-2 hover:text-primary transition-colors">
                  <Heart size={20} /> <span className="text-xs font-bold">Add to Wishlist</span>
               </button>
               <div className="w-px h-4 bg-surface-container-highest"></div>
               <button className="flex items-center gap-2 hover:text-primary transition-colors">
                  <ShoppingBag size={20} /> <span className="text-xs font-bold">Shipping Info</span>
               </button>
            </div>
          </motion.div>
        </div>
                 ...
                 <div key={idx} className="group cursor-pointer">
                    <div className={`polaroid p-2 pb-6 transition-all ${idx % 2 === 0 ? '-rotate-2' : 'rotate-1'} group-hover:rotate-0`}>
                       <div className="aspect-square bg-surface-container rounded-sm overflow-hidden mb-4">
                          <img src="/src/assets/images/product_keychain_1779100759746.png" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                       </div>
